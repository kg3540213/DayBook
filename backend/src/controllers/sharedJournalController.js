...
const createSharedJournal = async (req, res) => {
  ...
  try {
    // Check if a journal already exists between these two users
...
const acceptInvite = async (req, res) => {
  ...
  try {
    ...
    if (journal.status === "active")
      return res
        .status(400)
        .json({ message: "This journal is already active!" });
    if (journal.status === "declined")
      return res
        .status(400)
        .json({ message: "This invite was already declined!" });
...
const declineInvite = async (req, res) => {
  ...
  try {
    ...
    if (loggedUser.email !== journal.inviteEmail)
      return res
        .status(403)
        .json({ message: "This invite was not sent to your account." });
...
const getSharedJournal = async (req, res) => {
  ...
  try {
    ...
    if (!isMember(journal, loggedUser._id))
      return res
        .status(403)
        .json({ message: "You are not a member of this journal!" });
    if (journal.status !== "active")
      return res
        .status(403)
        .json({ message: "This journal is not active yet!" });
...
const deleteSharedJournal = async (req, res) => {
  ...
  try {
    ...
    // Only the owner can permanently delete; collaborator just leaves
    if (journal.owner.toString() === loggedUser._id.toString()) {
      ...
      return res
        .status(200)
        .json({ message: "Shared journal deleted permanently." });
    }

    // Collaborator leaving — detach them
    journal.collaborator = null;
    journal.status = "pending";
    await journal.save();
    return res
      .status(200)
      .json({ message: "You have left the shared journal." });
  } catch (error) {
...
const addSharedEntry = async (req, res) => {
  ...
  if (title.length > 20)
    return res
      .status(422)
      .json({ message: "Title cannot exceed 20 characters!" });
  if (content.length > 1500)
    return res
      .status(422)
      .json({ message: "Content cannot exceed 1500 characters!" });
  ...
  try {
    ...
    // Touch the journal's updatedAt for sorting
...
const updateSharedEntry = async (req, res) => {
  ...
  try {
    ...
    // Only the original author can edit their own entry
...
const deleteSharedEntry = async (req, res) => {
  ...
  try {
    ...
    // Author OR journal owner can delete an entry
...
// ── GET INVITE INFO (for accept/decline page) ─────────────────────
const getInviteInfo = async (req, res) => {
  const { token } = req.params;

  try {
    const journal = await SharedJournal.findOne({
      inviteToken: token,
    ...
    res.status(200).json({
      ...
      data: {
        ...
        ownerName:
          `${journal.owner.firstName} ${journal.owner.lastName || ""}`.trim(),
